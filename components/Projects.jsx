...
const getTypeIcon = (type) =>
  ({ web: 'ti-browser', mobile: 'ti-device-mobile', webapp: 'ti-layout-dashboard', logiciels: 'ti-code', cloud: 'ti-cloud-computing', ai: 'ti-brain' }[type] || 'ti-settings')

export default function Projects({ teaser }) {
  const all = getProjects().map((p) => ({ ...p, image: IMAGE_MAP[p.imageKey] || null }))
  const shown = all.slice(0, 3)
        ...
        <div className="container">
          ...
          <div className="projects-header">
            ...
            {teaser ? (
              <div className="projects-stats-teaser">
                <div className="stat-item-teaser">
                  <div className="stat-icon"><i className="ti ti-rocket" /></div>
                  <div className="stat-value-teaser">{all.length}+</div>
                  <div className="stat-label-teaser">Projets livrés</div>
                </div>
                <div className="stat-divider" />
                <div className="stat-item-teaser">
                  <div className="stat-icon"><i className="ti ti-star" /></div>
                  <div className="stat-value-teaser">100%</div>
                  <div className="stat-label-teaser">Satisfaction client</div>
                </div>
                <div className="stat-divider" />
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: p.image
                        ? 'none'
                        : `linear-gradient(135deg, ${getTypeColor(p.type)}, rgba(255,255,255,0.06))`,
                    }}
                  >
                    {p.image && (
                      <Image
                    ...
                    {p.image && (
                      <div className="thumb-overlay" style={{ background: `linear-gradient(135deg, ${getTypeColor(p.type)}dd, ${getTypeColor(p.type)}88)` }} />
                    )}
          ...
          {teaser && (
            <div className="projects-teaser-cta">
              <Link href="/projects" className="btn btn-primary projects-view-all">
                <span>Voir tous les projets</span>
                <i className="ti ti-arrow-right btn-arrow-icon" />
              </Link>
            </div>
          )}
        </div>
